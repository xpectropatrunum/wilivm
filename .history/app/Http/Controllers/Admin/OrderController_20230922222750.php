...
class OrderController extends Controller
{
    ...
    function update(Request $request, Order $order)
    {
        ...
        if($request->expiry_date){
            $order->update([
                "expires_at" => strtotime($request->expiry_date)
            ]);
        }

        $updated = $order->service->update($request->only("username", "ipv4", "ipv4_2", "ipv6", "password", "ip", "status", "label_ids", "cpu", "bandwith", "ram", "storage", "type", "plan", "os", "location"));
        $order->update([
            "label_ids" => $request->label_ids,
        ...
        if ($updated) {
            if ($request->upgrade) {
                ...
                if ($request->price <= 0) {
                    $wallet = $order->user->wallet;
                    $wallet->balance += -1 * $request->price;
                    $wallet->transaction()->create([
                    ...
                    if ($new = InvoiceItem::create([
                        "invoice_id" => $next_id,
                        "title" => "#{$order->id} upgrade to " . $order->service->type . " " .  $order->service->plan,
                        "discount" => 0,
                        "price" =>  $request->price,
                        "cycle" => $order->cycle,
                        "expires_at" =>  time() + $order->cycle * 86400 * 30,
                        "order_id" => $order->id,
                    ...
                    $email = Email::where("type", EEmailType::Upgrade_invoice)->first();
    ...
    {
        $labels = OrderLabel::where("enable", 1)->get();
        $expiry_date = date("Y-m-d H:i", $order->expires_at);

        return view("admin.pages.orders.edit", compact('order', 'labels', 'expiry_date'));
    }
