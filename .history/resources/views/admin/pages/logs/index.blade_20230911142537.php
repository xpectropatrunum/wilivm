                ...
                <div class="card-body p-3">
                    <form class="frm-filter" action="{{ route('admin.logs.index') }}" type="post" autocomplete="off">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div class="input-group input-group-sm" style="width: 70px;">
                                <select name="admind" class="custom-select">
                                    <option value="0" @if (!$admind) selected @endif>
                                        All</option>

                                    @foreach (App\Models\Admin::get() as $admin)
                                        <option value="{{ $admin->id }}"
                                            @if ($admind == $admin->id) selected @endif>{{ $admin->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @foreach (json_decode($item->related_id) as $key => $item_)
                                                    @if (!in_array($key, ['id', 'created_at', 'updated_at', 'password', 'deleted_at']) 
                                                    and strpos($key, '_id') === false and gettype($item_) == "string")
                                                        {{ $key }}: {{ $item_ }},
                                                    @endif
                                                @endforeach
                                            </td>
