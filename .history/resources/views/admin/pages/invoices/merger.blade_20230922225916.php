                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-lg-4">
                                ...
                                <select name="invoice_id[]" id="invoice_id" class="form-select select2" multiple>
                                    @if (request()->id)
                                        @foreach ($users->where('id', request()->id)->first()->invoices as $invoice)
        ...
        $(() => {
            ...
            $("[name=user_id]").change(function() {
                ...
                invoices.filter(i => i.user_id == $user_id).map(i => {
                    $("#invoice_id").append(`<option value="${i.id}">#${i.id}</option>`)
                })
