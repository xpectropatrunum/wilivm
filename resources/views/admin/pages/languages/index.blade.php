        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center px-3">
                    ...
                    <a class="btn btn-outline-primary btn-sm mx-3" href="{{ route('admin.languages.create') }}"><i class="fa fa-plus"></i> {{ __('admin.add_new') }}</a>
                </div>
                <div class="card-body p-3">
                    <form class="frm-filter" action="{{ route('admin.languages.index') }}" type="post" autocomplete="off">
                        @csrf
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-check">
                                            <input type="checkbox" data-url="{{ route('admin.languages.default',$item->id) }}" data-id="{{ $item->id }}" data-key="is_default" class="form-check-input changeStatus" id="exampleCheck{{ $item->id }}a" @if($item->is_default) checked @endif>
                                            <label class="form-check-label" for="exampleCheck{{ $item->id }}a"> yes</label>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-check">
                                            <input type="checkbox" data-url="{{ route('admin.languages.status',$item->id) }}" data-id="{{ $item->id }}" data-key="enable" class="form-check-input changeStatus" id="exampleCheck{{ $item->id }}" @if($item->enable) checked @endif>
                                            <label class="form-check-label" for="exampleCheck{{ $item->id }}"> enable</label>
                                        </div>
                                    </td>
                                    <td>{{ (new Shamsi)->jdate($item->created_at) }}</td>
                                    <td class="project-actions">
                                        <a class="btn btn-info btn-sm" href="{{ route('admin.languages.edit',$item->id) }}">
                                            <i class="fas fa-pencil-alt"></i>
                                            {{ __('admin.edit') }}
                                        </a>

                                        <form action="{{ route('admin.languages.destroy',$item->id) }}" class="d-inline-block" method="POST">
                                            @csrf
                                            @method('DELETE')
