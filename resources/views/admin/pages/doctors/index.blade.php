        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center px-3">
                    ...
                    <a class="btn btn-outline-primary btn-sm mx-3" href="{{ route('admin.doctors.create') }}"><i
                            class="fa fa-plus"></i> {{ __('admin.add_new') }}</a>
                </div>
                <div class="card-body p-3">
                    <form class="frm-filter" action="{{ route('admin.doctors.index') }}" type="post"
                        autocomplete="off">
                        @csrf
                        ...
                        <table class="table table-striped table-bordered text-nowrap mb-0">
                            ...
                            <tbody>
                                @foreach ($items as $item)
                                    <tr>
                                        <td>{{ $item->id }}</td>
                                        <td>{!! "<a target='_blank' href='" .route('admin.users.show', $item->user->id) . "'>Dr. ".$item->user->full_name."</a>"  !!}</td>
                                        <td>{!! $item->clinic ? ("<a target='_blank' href='" .route('admin.locations.clinics.index', ["search" => $item->clinic->name]) . "'>".$item->clinic->name."</a>") : "--" !!}</td>
                                        <td>{{ $item->specialty->title }}</td>
                                        <td>{{ (new Shamsi())->jdate($item->created_at) }}</td>
                                        <td>
                                            <div class="form-check">
                                                <input type="checkbox"
                                                    data-url="{{ route('admin.doctors.status', $item->id) }}"
                                        ...
                                        <td class="project-actions">
                                            <a class="btn btn-info btn-sm"
                                                href="{{ route('admin.doctors.edit', $item->id) }}">
                                                <i class="fas fa-pencil-alt"></i>
                                                {{ __('admin.edit') }}
                                            </a>

                                            <form action="{{ route('admin.doctors.destroy', $item->id) }}"
