                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-lg-3">
                                <label>Name</label>
                                <input type="text" value="{{ old('name', $email->name) }}" name="name"
                                    class="form-control @error('name') is-invalid @enderror" required>
                            </div>
                            <div class="form-group col-lg-3">
                                <label>Title (email subject)</label>
                                <input type="text" value="{{ old('title', $email->title) }}" name="title"
                                    class="form-control @error('title') is-invalid @enderror" required>
                            </div>
                            <div class="form-group col-lg-3">
                                <label>Head title (box header)</label>
                                <input type="text" value="{{ old('head', $email->head) }}" name="head"
                            ...
                            <div class="form-group col-lg-3">
                                ...
                                <select name="type" class="form-control select2">
                                    @foreach (App\Enums\EEmailType::asSelectArray()  as $key => $type)
                                        <option value="{{ $key }}"
                                            @if ($key == old('type', $email->type)) selected @endif>
                                            {{ $type }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group col-lg-8">
                                <textarea name="template" class="form-control editor" rows="10">{{ old('template', $email->template) }}</textarea>
                            ...
                            <div class="form-group col-lg-4">
                                ...
                                <div class="form-check">
                                    <input type="checkbox" name="enabled" class="form-check-input" value="1"
                                        id="exampleCheck2" @if (old('enabled', $email->enabled)) checked @endif>
