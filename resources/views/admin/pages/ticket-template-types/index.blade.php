...
@section('title', 'tickets template types')

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">tickets template types</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb @if (app()->getLocale() == 'fa') float-sm-left @else float-sm-right @endif">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('admin.dashboard') }}</a>
                </li>
                <li class="breadcrumb-item active">tickets template types</li>
            </ol>
        </div>
    </div>
@endsection
        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center px-3 justify-content-between">
                    <h3 class="card-title">tickets template types</h3>
                    <a class="btn btn-info btn-sm"
                    href="{{ route('admin.ticket-template-types.create') }}">
                    <i class="fas fa-plus"></i>
                    {{ __('Add Item') }}
                </a>
                </div>
                <div class="card-body p-3">
                    <form class="frm-filter" action="{{ route('admin.ticket-template-types.index') }}" type="post" autocomplete="off">
                        @csrf
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check">
                                                <input type="checkbox"
                                                    data-url="{{ route('admin.ticket-template-types.status', $item->id) }}"
                                        ...
                                        <td class="project-actions">
                                            <a class="btn btn-info btn-sm"
                                                href="{{ route('admin.ticket-template-types.edit', $item->id) }}">
                                                <i class="fas fa-edit"></i>
                                                {{ __('Edit') }}
                                            </a>
                                            <form action="{{ route('admin.ticket-template-types.destroy', $item->id) }}"
